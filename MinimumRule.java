 ...
 * 
 * Description: The MinimumRule class implements the maximum rule for finding Riemann sums.
 ...
 * Whichever height is smaller is the height used to calculate the area of the slice.
 * 
 * MinimumRule extends Riemann and implements the abstract methods 'slice' and 'slicePlot'.
 */
...
public class MinimumRule extends Riemann{
	...
	/**
	 * Calculates the area of the slice (between the graph of a polynomial and the x-axis, over a given interval on the x-axis) using the minimum hand rule.
	 * 
	 * @param poly
	...
	public double slice(polyfun.Polynomial poly, double sleft, double sright) {
		...
		//if the height of the rectangle from 'sleft' is greater than the height of the rectangle from 'sright', define height as being the height of the rectangle from 'sright'
		if (h1 > h2) 
		{
			height = h2;
		}
		...
		double areaminimumslice = (sright-sleft)*height; //defines areaminimumslice as the product of (b-a) and the polynomial evaluated at either 'sleft' or 'sright,' depending on which resulting height evaluated is smaller 
	    return areaminimumslice; //returns areaminimumslice
	}

	/**
	 * Graphs a slice using the minimum rule.
	 * 
	 *  @param pframe
